...
public class Aircraft {
    ...
    public int speed;
    double deg;
    // 飞机的血量，玩家操控的飞机血量为1，碰到就死弱鸡. 飞机的伤害，这个伤害属性在飞机发生碰撞时对敌方造成伤害, chp默认为2，
    // 且后续不打算修改，毕竟撞机不光荣
    public int hp = 10, maxHp = 10, chp = 20;
    public int shieldCharges = 0;
    // 飞机的buff， 两中buff， 2个子弹发射和三个子弹发射，当同时拥有两种buff时，这个buff将叠加，同时释放5个子弹,
    // 这个属性表示的是buff存在剩余时间，时间<0
    // 时，相应 buff 消失, 因为帧率是固定的，且每帧都会绘制自身，因此，tBuff1时间的减少不采用计时器，直接在draw里面减少即可
    // 他们的单位是秒
    double tBuff1 = -0.01, tBuff2 = -0.01;
    // 新增buff属性
    double tBuff5 = -0.01; // 增加子弹伤害持续时间
    double tBuff7 = -0.01; // 无敌时间持续时间
    double tBuff8 = -0.01; // 减速敌人持续时间
    int damageBonus = 0; // 伤害加成
    // 飞机的图像,死亡图像
    Image[] aircraft, death;
    // 图像索引, imgIndex <
    // aircraft.length时，正常绘制，当大于aircraft.length时，绘制death,当大于aircraft.length+绘制death.length时，表示飞机死亡，因为飞机已经绘制完成了死亡动画
    ...
    // 此处发射飞机的子弹, 发射自动是有如下情况，没有buff时的正常子弹，有一个buff时的增强子弹，两个buff同时存在的叠加超级强化子弹，
    // 本游戏正常子弹一种，增强子弹2种，强化叠加强化子弹1种
    // 超级强化就是牛
    ...
    // 增加子弹伤害
    public void boostDamage(double duration) {
        damageBonus = 1;
        tBuff5 = duration;
    }
    ...
    // 设置减速敌人时间
    public void setSlowEnemies(double duration) {
        tBuff8 = duration;
    }
    ...
    // 检查是否减速敌人
    public boolean isSlowingEnemies() {
        return tBuff8 > 0;
    }
    ...
    // 判断是否应该移除这个对象,如果飞机被击杀，或者飞机飞出屏幕外时，这个方法应该返回true,
    // 敌方飞机的生成实在屏幕上方，所以不判断屏幕上方的飞机,况且敌方飞机不会向上飞,至少不会垂直向上
    // 这里更改为， 移动到双倍的屏幕外才会移除， 即在 (-WIDTH, -HEIGHT, WIDTH*2, HEIGHT*2) 的范围外时，会被判定离开屏幕
    public boolean isRemove() {
        // 此处有三种情况会返回 true
        return ((hp <= 0) && (imgIndex >= (aircraft.length + death.length) * fps)) || (x > Data.WIDTH * 2
                || x < -width + -Data.WIDTH || y > Data.HEIGHT * 2 || y < -height + -Data.HEIGHT);
    }

    public void draw(Graphics g) {
        // 这个fps表示每隔多少帧更新一下图片，因为如果没一帧都更新，在帧率为50的情况下，每秒切换50次，会很鬼畜
        if (imgIndex >= (aircraft.length + death.length) * fps)
            return;
        // 更新 imgIndex
        imgIndex++;
        ...
        /*
         * 如果想让其播放死亡动画，则直接让imgIndex = aircraft.length * fps + 1, 此时这个方法播放在调用
         * death.length *fps 次后, draw将不会被执行
         */
